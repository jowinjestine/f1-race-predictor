...
# ---------------------------------------------------------------------------
# Train Model G (Temporal Sequence) on a GCE GPU VM, then upload results.
#
# Usage:  bash scripts/run_model_g_remote.sh [--cpu]
# ---------------------------------------------------------------------------

PROJECT="jowin-personal-2026"
ZONE="${ZONE:-us-central1-a}"
BUCKET="f1-predictor-artifacts-jowin"
VM_NAME="f1-model-g-$(date +%s)"
...
# ---------------------------------------------------------------------------
# 2. Create the startup script (Model G only)
# ---------------------------------------------------------------------------
STARTUP_SCRIPT=$(cat <<'STARTUP_EOF'
#!/bin/bash
set -euo pipefail
exec > /var/log/f1-training.log 2>&1
echo "=== Model G training begin: $(date) ==="
...
# Run Model G only
echo "=== Running Model G: $(date) ==="
uv run jupyter nbconvert --to notebook --execute \
    --ExecutePreprocessor.timeout=10800 \
    notebooks/05g_model_G_temporal.ipynb \
    --output 05g_model_G_temporal.ipynb 2>&1 || true

# Upload results to GCS
echo "=== Uploading results: $(date) ==="
gsutil -m -q cp data/training/model_G_*.parquet "gs://$BUCKET/$RESULTS/training/" 2>/dev/null || true
gsutil -m -q cp data/raw/model/Model_G_*.pkl "gs://$BUCKET/$RESULTS/raw/model/" 2>/dev/null || true
gsutil -q cp notebooks/05g_model_G_temporal.ipynb "gs://$BUCKET/$RESULTS/notebooks/" 2>/dev/null || true

# Signal completion
echo "DONE" | gsutil -q cp - "gs://$BUCKET/$STAGING/MODEL_G_DONE"

echo "=== Model G training complete: $(date) ==="
...
STARTUP_FILE="/tmp/f1-model-g-startup.sh"
...
echo ""
echo "============================================================"
echo "  VM '$VM_NAME' created — training Model G only"
echo "============================================================"
echo ""
echo "  Monitor:"
...
echo ""
echo "  Check if done:"
echo "    gsutil stat gs://$BUCKET/$STAGING_PREFIX/MODEL_G_DONE"
echo ""
echo "  Manual cleanup:"
echo "    gcloud compute instances delete $VM_NAME --zone=$ZONE --quiet"
echo ""
echo "  Estimated time: ~120-180 min (10 sequence models + Optuna)"
echo "============================================================"
