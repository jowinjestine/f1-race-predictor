...
# ---------------------------------------------------------------------------
# Train Model H (Delta + Monte Carlo) on a GCE GPU VM, then upload results.
#
# Usage:  bash scripts/run_model_h_remote.sh [--cpu]
# ---------------------------------------------------------------------------

PROJECT="jowin-personal-2026"
ZONE="${ZONE:-us-central1-a}"
BUCKET="f1-predictor-artifacts-jowin"
VM_NAME="f1-model-h-$(date +%s)"
...
# ---------------------------------------------------------------------------
# 2. Create the startup script (Model H only)
# ---------------------------------------------------------------------------
STARTUP_SCRIPT=$(cat <<'STARTUP_EOF'
#!/bin/bash
set -euo pipefail
exec > /var/log/f1-training.log 2>&1
echo "=== Model H training begin: $(date) ==="
...
# Run Model H only
echo "=== Running Model H: $(date) ==="
uv run jupyter nbconvert --to notebook --execute \
    --ExecutePreprocessor.timeout=7200 \
    notebooks/05h_model_H_delta_mc.ipynb \
    --output 05h_model_H_delta_mc.ipynb 2>&1 || true

# Upload results to GCS
echo "=== Uploading results: $(date) ==="
gsutil -m -q cp data/training/model_H_*.parquet "gs://$BUCKET/$RESULTS/training/" 2>/dev/null || true
gsutil -m -q cp data/raw/model/Model_H_*.pkl "gs://$BUCKET/$RESULTS/raw/model/" 2>/dev/null || true
gsutil -q cp notebooks/05h_model_H_delta_mc.ipynb "gs://$BUCKET/$RESULTS/notebooks/" 2>/dev/null || true

# Signal completion
echo "DONE" | gsutil -q cp - "gs://$BUCKET/$STAGING/MODEL_H_DONE"

echo "=== Model H training complete: $(date) ==="
...
STARTUP_FILE="/tmp/f1-model-h-startup.sh"
...
echo ""
echo "============================================================"
echo "  VM '$VM_NAME' created — training Model H only"
echo "============================================================"
echo ""
echo "  Monitor:"
...
echo ""
echo "  Check if done:"
echo "    gsutil stat gs://$BUCKET/$STAGING_PREFIX/MODEL_H_DONE"
echo ""
echo "  Manual cleanup:"
