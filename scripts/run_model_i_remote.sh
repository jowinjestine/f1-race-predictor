...
# ---------------------------------------------------------------------------
# Train Model I (Quantile/Uncertainty) on a GCE GPU VM, then upload results.
#
# Usage:  bash scripts/run_model_i_remote.sh [--cpu]
# ---------------------------------------------------------------------------

PROJECT="jowin-personal-2026"
ZONE="${ZONE:-us-central1-a}"
BUCKET="f1-predictor-artifacts-jowin"
VM_NAME="f1-model-i-$(date +%s)"
...
# ---------------------------------------------------------------------------
# 2. Create the startup script (Model I only)
# ---------------------------------------------------------------------------
STARTUP_SCRIPT=$(cat <<'STARTUP_EOF'
#!/bin/bash
set -euo pipefail
exec > /var/log/f1-training.log 2>&1
echo "=== Model I training begin: $(date) ==="
...
uv pip install xgboost lightgbm optuna scipy
...
# Run Model I only
echo "=== Running Model I: $(date) ==="
uv run jupyter nbconvert --to notebook --execute \
    --ExecutePreprocessor.timeout=7200 \
    notebooks/05i_model_I_quantile.ipynb \
    --output 05i_model_I_quantile.ipynb 2>&1 || true

# Upload results to GCS
echo "=== Uploading results: $(date) ==="
gsutil -m -q cp data/training/model_I_*.parquet "gs://$BUCKET/$RESULTS/training/" 2>/dev/null || true
gsutil -m -q cp data/raw/model/Model_I_*.pkl "gs://$BUCKET/$RESULTS/raw/model/" 2>/dev/null || true
gsutil -q cp notebooks/05i_model_I_quantile.ipynb "gs://$BUCKET/$RESULTS/notebooks/" 2>/dev/null || true

# Signal completion
echo "DONE" | gsutil -q cp - "gs://$BUCKET/$STAGING/MODEL_I_DONE"

echo "=== Model I training complete: $(date) ==="
...
STARTUP_FILE="/tmp/f1-model-i-startup.sh"
...
echo ""
echo "============================================================"
echo "  VM '$VM_NAME' created — training Model I only"
echo "============================================================"
echo ""
echo "  Monitor:"
...
echo ""
echo "  Check if done:"
echo "    gsutil stat gs://$BUCKET/$STAGING_PREFIX/MODEL_I_DONE"
echo ""
echo "  Manual cleanup:"
echo "    gcloud compute instances delete $VM_NAME --zone=$ZONE --quiet"
echo ""
echo "  Estimated time: ~60-120 min"
echo "============================================================"
